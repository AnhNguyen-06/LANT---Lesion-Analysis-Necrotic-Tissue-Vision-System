...
1. **Pixel-to-Physical Calibration**:
   $$\text{Ratio} = \frac{\text{Known Dimension (cm)}}{\text{Marker Pixels}}$$
   $$\text{Area } (cm^2) = \text{Wound Mask Pixels} \times \text{Ratio}^2$$

2. **Tissue RYB Composition**:
   $$\text{Total Mask} = P_{\text{Red}} + P_{\text{Yellow}} + P_{\text{Black}} + P_{\text{Pink}} = 100\%$$
...
5. **Smart Critical Alert Thresholds**:
   - **EMERGENCY CRITICAL**: $\%Black \ge 10\%$ OR $\%Yellow \ge 35\%$
